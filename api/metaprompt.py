"""
Metaprompt engine — ported from Anthropic's Prompt Generator notebook.

This module contains the full metaprompt text and all utility functions
to generate, extract, clean, and test prompt templates using Google Gemini.
"""
...
# ---------------------------------------------------------------------------
# The Metaprompt (exact port from Anthropic's notebook)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Prompt for fixing floating variables (exact port from notebook)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Utility functions (ported from the notebook)
# ---------------------------------------------------------------------------
...
def remove_empty_tags(text: str) -> str:
    """Remove XML tag pairs that contain only whitespace."""
    return re.sub(r'\n<(\w+)>\s*</\1>\n', '', text, flags=re.DOTALL)


def strip_last_sentence(text: str) -> str:
    """Remove trailing 'Let me know...' sentences."""
...
def extract_prompt(metaprompt_response: str) -> str:
    """Extract the <Instructions> block from the metaprompt response."""
    ...
    return content[:1000] + strip_last_sentence(
        remove_empty_tags(
            remove_empty_tags(content[1000:]).strip()
        ).strip()
    )


def extract_variables(prompt: str) -> list[str]:
    """Extract all {$VARIABLE_NAME} references from a prompt."""
...
def find_free_floating_variables(prompt: str) -> list[str]:
    """Find variables used outside of XML tags (free-floating)."""
...
def generate_prompt(task: str, variables: list[str] | None = None) -> dict:
    """
    Generate a prompt template for the given task using the metaprompt.

    This is the core function that mirrors the Anthropic notebook flow:
    1. Insert the task into the metaprompt
    2. Optionally pre-fill the variables
    3. Call Gemini to generate the prompt
    4. Extract the prompt template and variables
    5. Fix any floating variables
    ...
    Returns:
        Dict with 'prompt', 'variables', and 'raw_response' keys.
    """
    # Build the prompt by inserting the task
    ...
    extracted_prompt = extract_prompt(raw_response)
...
def test_prompt(prompt_template: str, variable_values: dict[str, str]) -> str:
    """
    Test a generated prompt by substituting variable values and running it.
    ...
    Returns:
        The AI's response to the filled-in prompt.
    """
...
def improve_prompt(
    ...
    """
    Improve an existing prompt template based on feedback.

    Args:
        prompt: The existing prompt template to improve.
...
"""
    ...
    improved = extract_between_tags("improved_prompt", response)
    if improved:
        improved_prompt = improved[0].strip()
    else:
